...
| P1-5 | PWA | **SW push deep-link hardcodes `locale='en'` — FIXED + VERIFIED**: SW now reads `data.locale` (fallback en); API stores per-subscription locale (`push_subscriptions.locale`, migration 0016) captured at subscribe time from `useLocale()`; `sendPushToUsers` injects the subscriber's locale into each push payload. Live-verified: subscribe with `locale:"ar"` → stored `ar` in DB. | apps/player-pwa/worker/index.js:19; notifications.service.ts:125-166; usePushNotifications.ts | **DONE ✅** (parent session 2026-08-26T18:2xZ; tsc 0, jest 30/30, vitest 217/217, build 3/3, migration 0016 applied) | docs/plans/ (P1-5 slice, parent session) |
