...
export class MatchesService {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Player Reviews — post-match rating & feedback
  // ─────────────────────────────────────────────────────────────────────────

  /**
   * Submit reviews for fellow players after a completed match.
   * Accepts an array of {reviewee_id, rating (1-5), comment?}.
   * Validates: match is completed, reviewer attended, not reviewing self,
   * not reviewing same player twice for same match.
   */
  async submitReviews(
    reviewerId: string,
    matchId: string,
    reviews: { revieweeId: string; rating: number; comment?: string }[],
  ) {
    ...
    if (match.status !== 'Completed') {
      throw new BadRequestException('Can only review completed matches.');
    }

    // Verify reviewer attended
    const [membership] = await this.db
      .select({ id: match_players.id })
      .from(match_players)
      .where(
        sql`${match_players.match_id} = ${matchId} AND ${match_players.user_id} = ${reviewerId}`,
      )
      .limit(1);

    if (!membership) {
      throw new BadRequestException('You did not attend this match.');
    }
    ...
    for (const r of reviews) {
      ...
      // Verify reviewee attended
      const [attended] = await this.db
        .select({ id: match_players.id })
        .from(match_players)
        .where(
          sql`${match_players.match_id} = ${matchId} AND ${match_players.user_id} = ${r.revieweeId}`,
        )
        .limit(1);

      if (!attended) {
        throw new BadRequestException(
          `Player ${r.revieweeId} did not attend this match.`,
        );
      }
    }

    // Insert all reviews (upsert for idempotency)
    await this.db.transaction(async (tx) => {
      for (const r of reviews) {
        await tx
          .insert(schema.match_reviews)
          .values({
            match_id: matchId,
          ...
          .onConflictDoUpdate({
            target: [
              schema.match_reviews.match_id,
              schema.match_reviews.reviewer_id,
              schema.match_reviews.reviewee_id,
            ],
            set: { rating: r.rating, comment: r.comment ?? null, created_at: new Date() },
          });
      }
    });

    return { reviewed: reviews.length, message: 'Reviews submitted.' };
  }
  ...
  async getMatchReviews(matchId: string) {
    return this.db
      .select({
        id: schema.match_reviews.id,
        reviewerId: schema.match_reviews.reviewer_id,
        revieweeId: schema.match_reviews.reviewee_id,
        rating: schema.match_reviews.rating,
        comment: schema.match_reviews.comment,
        createdAt: schema.match_reviews.created_at,
      })
      .from(schema.match_reviews)
      .where(eq(schema.match_reviews.match_id, matchId))
      .orderBy(schema.match_reviews.created_at);
  }
}
