...
import { X, Trophy, Star, Users, Loader2, AlertTriangle } from 'lucide-react';
            ...
            <div className="fixed bottom-0 inset-x-0 z-[70] bg-white rounded-t-3xl max-w-md mx-auto animate-slide-up max-h-[70vh] overflow-y-auto">
                ...
                <div className="px-5 pb-8">
                    <div className="flex flex-col items-center pt-2 pb-5">
                        ...
                        {profile?.handle && <p className="text-xs text-gray-400" dir="ltr">@{profile.handle}</p>}
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 mb-5">
                        {player.isHost && (
                            <span className="text-[10px] font-bold text-brand-green bg-brand-green/10 px-3 py-1.5 rounded-full flex items-center gap-1">
                                <Trophy className="w-3 h-3" strokeWidth={2.5} />{t('matchDetail.organizer')}
                            </span>
                        )}
                        {player.team && (
                    ...
                    {error && (
                        <div className="flex flex-col items-center py-4">
                            <AlertTriangle className="w-5 h-5 text-gray-300 mb-1" strokeWidth={1.5} />
                    ...
                    {profile && !isLoading && (
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-gray-50 rounded-xl p-3 text-center">
                                ...
                                <p className="text-sm font-bold text-brand-black" dir="ltr">{profile.rating?.toFixed(1) ?? '—'}</p>
                            ...
                            <div className="bg-gray-50 rounded-xl p-3 text-center">
                                ...
                                <p className="text-sm font-bold text-brand-black" dir="ltr">{profile.pom_count ?? 0}</p>
                                <p className="text-[10px] text-gray-400">{t('profile.pomCount')}</p>
                            </div>
                            <div className="bg-gray-50 rounded-xl p-3 text-center">
                                <Users className="w-4 h-4 text-gray-400 mx-auto mb-1" strokeWidth={2} />
                                <p className="text-sm font-bold text-brand-black" dir="ltr">{profile.games_played ?? 0}</p>
