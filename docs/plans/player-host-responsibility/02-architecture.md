...
| `apps/player-pwa/src/components/payment/PaymentSheet.tsx` | Balance preview stays; pay action now calls `POST /matches/:id/join {idempotencyKey}` (no /wallet/pay). NEW joiner responsibility notice directly above the pay button (Abdullah 2026-09-09: joiners MUST see who operates the match before paying): self-booked → amber `payment.selfHostedNotice` ("booked and operated fully by this host — KoraLink is the platform only"); koralink player-hosted → neutral `payment.playerHostedNotice`; venue-owned → nothing |
...
| `apps/player-pwa/src/app/[locale]/match/[id]/page.tsx` | Mode-aware responsibility banner: self = amber "run entirely by the host — all pitch ops are his responsibility"; koralink = neutral "KoraLink booked the pitch and will resolve problems the host can't" + payout/refund status lines (host vs joiner). `OngoingGameJoinSheet` carries the same one-line warning |
